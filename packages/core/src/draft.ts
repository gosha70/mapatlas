...
function cloneState(state: DraftState): DraftState {
  return { points: state.points.map(cloneDraftPoint), breaks: [...state.breaks] };
}
...
export function createTrackDraft(from?: Track): TrackDraft {
  const initial: DraftState = { points: [], breaks: [] };
  ...
  return {
    ...
    setTimeAt: (index, t) => {
      ...
      if (!Number.isInteger(t) || t < 0) {
        throw new RangeError(`setTimeAt: ${t} is not an epoch-millisecond timestamp`);
      }
      const next = cloneState(current);
      const point = next.points[index];
    ...
    toTrack: (meta, policy) => {
      ...
      const points: TrackPoint[] = current.points.map((p) => {
        const { t, ...rest } = p;
        if (t === undefined) throw new TrackDraftIncompleteError(untimed());
        return { ...rest, t };
      });
      ...
      return finalizeTrack(
        {
          ...
          ...(meta?.meta === undefined ? {} : { meta: structuredClone(meta.meta) }),
        },
        policy,
      );
    },
...
function interpolate(state: DraftState, options: InterpolateTimesOptions): DraftState {
  ...
  for (let i = 1; i < points.length; i += 1) {
    const a = points[i - 1];
    const b = points[i];
    const step = a === undefined || b === undefined ? 0 : haversineDistanceMeters(a, b);
